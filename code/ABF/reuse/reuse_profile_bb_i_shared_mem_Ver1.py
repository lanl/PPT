    ...
    for i in range(0,len(bb_table)):
        ...
        for i in range (0, core_count):
            ...
            for bb_i_size in bbi_win_sizes[i]:
                res_sds[i] = np.concatenate([res_sds[i], get_bbi_reuse_prof_fast(bb_i_size,orig_trace)])
            print res_sds[i]
            zipped_sd_psd_i[i] = get_prob_sd_bb_i(res_sds[i])
            res_sd_psd_i[i] = [list(sd_psd) for sd_psd in zipped_sd_psd_i[i]]
            print "Conditional private reuse profile of core ", i, " basic block " , b_t_info[0], " is ", res_sd_psd_i[i]
            #print "Probability of BasicBlock ",b_t_info[2]," is ", b_t_info[-1]
            f_res_sd_psd_i[i] = get_final_prob_bb_i(res_sd_psd_i[i], float(b_t_info[-1])/core_count)
            print "Weighted probability of BasicBlock", b_t_info[2], " core ", i ," is ",f_res_sd_psd_i[i]
            for f_sd_psd_i in f_res_sd_psd_i[i]:
