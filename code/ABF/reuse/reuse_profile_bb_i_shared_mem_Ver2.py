...
File            :       reuse_profile_bb_i_shared_mem_Ver1.py
...
def usuage():
    print "[USUAGE]: "+__file__+" BasicBlocks_table mem_trace num_cores"
...
def get_bbi_reuse_prof_fast(bb_i_size, orig_trace):
    ...
    if bb_i_size[1] > bb_i_size[0]+1:
        bbi_trace = orig_trace[bb_i_size[0]+1:bb_i_size[1]-1]
        sd = []     
        # Calculate the SDs for each addr in BBi
        for addr, idx in zip(bbi_trace,range(bb_i_size[0]+1,bb_i_size[1]-1)):
            window_trace = orig_trace[:idx-1]
            dict_sd = {}
            addr_found = False
...
def merge_reuse_profile(final_reuse_profile):
    '''
...
def main(bb_file,trace_file, num_cores):
    ...
    orig_trace = read_prep_trace_from_file('processed_trace.dat')
            
    #trace_dict = get_trace_dict(orig_trace)    
    trace_dict = read_trace_dict_from_file ('trace_dict.dat')
    ...
    for i in range(0,len(bb_table)):
        ...
        print "\nBasicblock Name: ", b_t_info[2], " Probability: ", b_t_info[-1]

        if("for.body" in b_t_info[2]):
            print "*************This basic blocks will be split by openmp*************"
        
        bbi_win_sizes = {} 
        bbi_win_sizes[0] = get_bbi_windows(b_t_info[2],trace_dict)
        core_count = 1
        if("for.body" in b_t_info[2]):
            core_count = (int)(num_cores)
            sampled_bbi_win_sizes = sample_bbi_wins(bbi_win_sizes[0],sample_size*core_count)
            for i in range (0, core_count):
                bbi_win_sizes[i] = sampled_bbi_win_sizes[i::core_count]
        else:
            bbi_win_sizes[0] = sample_bbi_wins(bbi_win_sizes[0], sample_size)
        #print "Sampled bb", i, " windows : ", bbi_win_sizes
                
        final_reuse_prof_per_bb = []
        for i in range (0, core_count):
            res_sds = np.array([])
            for bb_i_size in bbi_win_sizes[i]:
            ...
            #print "Conditional private reuse profile of core ", i, " basic block " , b_t_info[0], " is ", res_sd_psd_i
            #print "Probability of BasicBlock ",b_t_info[2]," is ", b_t_info[-1]
            f_res_sd_psd_i = get_final_prob_bb_i(res_sd_psd_i, float(b_t_info[-1])/core_count)
            #print "Weighted probability of BasicBlock", b_t_info[2], " core ", i ," is ",f_res_sd_psd_i
            
            for f_sd_psd_i in f_res_sd_psd_i:
                final_reuse_profile.append(f_sd_psd_i)
                final_reuse_prof_per_bb.append(f_sd_psd_i)

            #print "Stack Distance Stats of BasicBlock ", b_t_info[2]
            #print "mean %f min %f max %f std %f" %(np.mean(res_sds[i]),np.min(res_sds[i]),np.max(res_sds[i]),np.std(res_sds[i]))
        if("for.body" in b_t_info[2]):
            with open("for.body_reuse_profile_"+num_cores+".dat","w") as f:
                for key, value in merge_reuse_profile(final_reuse_prof_per_bb).iteritems():
    ...
    all_bbi_sd_profile = merge_reuse_profile(final_reuse_profile)
    ...
    with open(bb_file[:-4]+"_reuse_profile_"+num_cores+".dat","w") as f:
