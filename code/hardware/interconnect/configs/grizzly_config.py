...
'''
We got to change the values of these parameters,
the current parameter values belong to moonlight.
I am changing them one by one

Grizzly use Intel's OmniPath interconnect, we need to model that.
'''
grizzly_intercon = {
    "num_ports_per_switch": 48, # m-port
    "num_levels": 2, # n-tree
    "switch_link_up_delay": 0.7e-6,
    "switch_link_down_delay": 0.7e-6,
    "host_link_delay": 0.7e-6,
    "switch_link_up_bdw": 40e9,  #56GB/s
    "switch_link_down_bdw": 40e9,
    "host_link_bdw": 40e9, #56Gb/s
    "switch_link_dups": 1,
