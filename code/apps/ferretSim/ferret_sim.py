...
simName, startTime, endTime, minDelay, useMPI = \
  "ferretsim", 0.0, 100000000000.0, 0.1, False
...
def FerretSim(mpi_comm_world, *args):
    sim = sys.argv[1]
...
#     probability_sd = [0.1, 0.5, 0.1, 0.2, 0.05, 0.05] # probability distributions
    ...
    block_size = 8.0 # in mem accesses (can be a distribution, but for us ONLY AVERAGE MATTERS)

    data_bus_width = 4.0 #in Bytes
    
    total_bytes = 669700828 # simdev
    #simdev i/p
    tasklist = [['iALU', 19051188], ['fALU', 12955765], ['fDIV',696482], ['MEM_ACCESS', 4, 10, 1, 1, 1, 10, 80, False, stack_dist, probability_sd, block_size, total_bytes, data_bus_width]]
    
    #total_bytes = 12203578546 #simsmall
    #simsmall i/p
    #tasklist = [['iALU', 305713547], ['fALU', 184765648], ['fDIV', 6319785], ['MEM_ACCESS', 4, 10, 1, 1, 1, 10, 80, False, stack_dist, probability_sd, block_size, total_bytes, data_bus_width]]
...
modeldict = { "model_name"    : "ferretsim",
              "sim_time"      : 1.0*10**16,
              "use_mpi"       : False,
...
hostmap = range(1)
cluster.start_mpi(hostmap, FerretSim)
cluster.run()