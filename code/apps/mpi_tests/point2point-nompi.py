...
if len(sys.argv) != 6 and len(sys.argv) != 7:
    print("Usage: point2point-nomopi.py gemini_dims total_ranks ranks_per_host iat data_size pattern [sample_interval]")
    sys.exit(1)
...
if pattern == '2dnn':
    ...
    if n1*n1*n1 != n:
        ...
        sys.exit(1)
print("point2point-nompi.py dims=%d n=%d x=%d iat=%d sz=%d pattern=%s samp=%d" % 
      (dims, n, x, iat, sz, pattern, samp))

modeldict = {
    "model_name" : "point2point-nompi",
    "sim_time" : 10,
    "use_mpi" : False,
    "intercon_type" : "Gemini",
    "host_type" : "Host",
    "torus" : configs.gemini_anydim(dims, dims, dims),
...
cluster = Cluster(modeldict, debug_options={"hpcsim", "torus", "switch"})
