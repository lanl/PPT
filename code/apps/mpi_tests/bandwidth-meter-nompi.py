...
if len(sys.argv) != 5:
    print("Usage: python bandwidth-meter.py host0 host1 parallel_sessions data_size")
    sys.exit(1)
...
ps = int(sys.argv[3])
sz = int(sys.argv[4])
print("python bandwidth-meter.py %d %d %d %d" % (h0, h1, ps, sz))
...
cluster = Cluster(model_dict, model_name="bandwidth-meter", sim_time=1e9, use_mpi=False)
