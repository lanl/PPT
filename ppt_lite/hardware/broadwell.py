...
#broadwell.py 
''' 
  A Simian resource that represents broadwell core. It has
...
clockspeed = ( 
        3.20 * 10 ** 9  # next highest clock 3.10 Highest Turbo Clocks  3.80 3.60 
...
cache_levels = 3 
cache_sizes = [32 * kb, 256 * kb, 46080 * kb]  # bytes 
...
# cache_cycles = [4.0, 12.0, 65.0]  # cycles from www.7-cpu.com where they can be found 
cache_cycles = [4.0, 12.0, 34.0]  # cycles from microarhitechture.pdf from agner fog 
...
pipelinelatencies = {  # in seconds 
        ...
        "idiv": 71.000 / clockspeed, 
        "fdiv": 12.500 / clockspeed, 
...
pipelinethroughputs = {  # in seconds 
        "iadd": 0.250 / clockspeed, 
        "fadd": 1.000 / clockspeed, 
        "idiv": 28.500 / clockspeed, 
