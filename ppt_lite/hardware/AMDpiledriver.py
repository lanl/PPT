...
#AMDpiledriver.py 
''' 
  A Simian resource that represents AMDpiledriver core. It has
...
clockspeed = ( 
        4.70 * 10 ** 9  # next highest clock 4.40 Highest Turbo Clocks  5.00 4.70 
...
cache_levels = 3 
cache_sizes = [16 * kb, 2048 * kb, 8192 * kb]  # bytes 
cache_line_sizes = [64, 64, 64 ]  # bytes 
associativity = [4, 16, 64] 
num_cache_lines = [ 
...
cache_cycles = [3.5, 20.0, 87.0]  # cycles from microarhitechture.pdf from agner fog 
...
pipelinelatencies = {  # in seconds 
        ...
        "fadd": 5.500 / clockspeed, 
        "idiv": 42.000 / clockspeed, 
        "fdiv": 24.500 / clockspeed, 
        "imul": 6.000 / clockspeed, 
        "fmul": 5.500 / clockspeed, 
...
pipelinethroughputs = {  # in seconds 
        ...
        "idiv": 42.000 / clockspeed, 
        "fdiv": 10.000 / clockspeed, 
        "imul": 4.000 / clockspeed, 
