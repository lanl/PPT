...
#knl.py 
''' 
  A Simian resource that represents knl core. It has
...
clockspeed = ( 
        1.50 * 10 ** 9  # next highest clock 1.40 Highest Turbo Clocks  1.70 1.60 
...
cache_levels = 3 
cache_sizes = [32 * kb, 1024 * kb, 16777216 * kb]  # bytes 
cache_line_sizes = [64, 64, 64 ]  # bytes 
associativity = [8, 16, 1] 
num_cache_lines = [ 
...
cache_cycles = [4.0, 17.0, 200.0]  # cycles from microarhitechture.pdf from agner fog 
...
pipelinelatencies = {  # in seconds 
        ...
        "fadd": 6.000 / clockspeed, 
        "idiv": 80.500 / clockspeed, 
        "fdiv": 41.000 / clockspeed, 
        "imul": 5.000 / clockspeed, 
        "fmul": 7.000 / clockspeed, 
...
pipelinethroughputs = {  # in seconds 
        "iadd": 1.000 / clockspeed, 
        "fadd": 1.500 / clockspeed, 
        "idiv": 71.500 / clockspeed, 
        "fdiv": 37.000 / clockspeed, 
