...
#AMDryzen.py 
''' 
  A Simian resource that represents AMDryzen core. It has
...
clockspeed = ( 
        3.80 * 10 ** 9  # next highest clock 3.70 Highest Turbo Clocks  4.50 4.30 
...
cache_levels = 3 
cache_sizes = [32 * kb, 512 * kb, 16384 * kb]  # bytes 
...
# cache_cycles = [4.0, 12.0, 38.0]  # cycles from www.7-cpu.com where they can be found 
cache_cycles = [4.0, 17.0, 40.0]  # cycles from microarhitechture.pdf from agner fog 
...
pipelinelatencies = {  # in seconds 
        ...
        "fadd": 5.000 / clockspeed, 
        "idiv": 30.500 / clockspeed, 
        "fdiv": 11.500 / clockspeed, 
        "imul": 3.000 / clockspeed, 
        "fmul": 5.000 / clockspeed, 
        "load": 0.100 / clockspeed, 
        "store": 4.000 / clockspeed, 
...
pipelinethroughputs = {  # in seconds 
        ...
        "idiv": 29.500 / clockspeed, 
        "fdiv": 5.000 / clockspeed, 
